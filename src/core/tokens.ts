...
export async function setupPaseto() {
    try {
        await access(
            getDataDir(),
            fsConstants.W_OK | fsConstants.R_OK | fsConstants.F_OK,
        );
    } catch {
    ...
    try {
        ...
        try {
            ...
            const newKey = paseto.keyObjectToBytes(
                await paseto.generateKey('public'),
            );
            await writeFile(
                `${getDataDir()}/key.pem`,
            ...
            log('error', `Error generating and saving key: ${error.message}`);
            process.exit(1);
        }
    }
}
...
export async function initPaseto(cli = false) {
    try {
        ...
        log('error', `Error reading key for authentication: ${error.message}`);
        process.exit(1);
    }
    ...
    if (revokedFileExists) {
        try {
            revokedTokens = JSON.parse(
                await readFile(`${getDataDir()}/revoked-tokens.json`, 'ascii'),
            );
 ...
 * @returns
 */
export async function generateDeployToken(
